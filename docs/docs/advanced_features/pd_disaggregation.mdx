...
ASCEND_MF_TRANSFER_PROTOCOL selects the transport (`sdma` by default; `host_rdma` / `device_rdma` / `device_urma` with newer memfabric_hybrid). `host_rdma` additionally requires ASCEND_MF_HCOM_URL pointing at the RDMA nic data-plane endpoint; each worker derives its own port from this base (Prefill and Decode get disjoint ranges, so both roles can share one host).

```bash
export ASCEND_MF_TRANSFER_PROTOCOL=host_rdma
export ASCEND_MF_HCOM_URL="tcp://<rdma-nic-ip>:<port>"
```
